// Qudrat unified math renderer v87 — fractions, Arabic RTL roots and powers.
(function(){
 ...
 function rootInner(x){
  ...
  const term=t=>{const v=String(t||'').trim();const m=v.match(/^([0-9٠-٩۰-۹]+)(?:\s*\^\s*([0-9٠-٩۰-۹]+)|([⁰¹²³⁴⁵⁶⁷⁸⁹]+))$/);return m?pow(m[1],m[2]||cv(m[3])):`<span class="qrootterm">${esc(ar(v))}</span>`};
  const parts=s.split(/\s*([+\-−×÷])\s*/).filter(v=>v!=='');
  const nodes=parts.map((p,i)=>i%2?`<span class="qrootop">${esc(p)}</span>`:term(p));
  return `<span class="qrootexpr" dir="rtl">${nodes.join('')}</span>`
 }
 function sqrt(x){return `<bdi dir="rtl" class="qmath-isolate qroot-isolate"><span class="qsqrt qsqrt-rtl"><span>${rootInner(x)}</span></span></bdi>`}
 ...
 function render(raw){
  ...
  s=s.replace(/\{\{\s*chart\s*:\s*bar\s*:\s*([^}]+)\}\}/gi,(_,x)=>hold(chart(x))).replace(/\{\{shape:(triangle|rect|rectangle|circle|square)(?::([^}]+))?\}\}/gi,(_,t,a)=>hold(svg(t.toLowerCase(),a?a.split(':'):[]))).replace(/QVISUALTOKEN\s*[·.،,:-]*\s*(triangle|rect|rectangle|circle|square)?/gi,(_,t)=>hold(svg((t||'rect').toLowerCase(),[]))).replace(/\{\{frac:([^}:]+):([^}]+)\}\}/gi,(_,a,b)=>hold(frac(a,b))).replace(/\{\{sqrt:([^}]+)\}\}/gi,(_,x)=>hold(sqrt(x))).replace(/\{\{pow:([^}:]+):([^}]+)\}\}/gi,(_,a,b)=>hold(pow(a,b)));
  s=s.replace(/√\s*[（(]\s*([^()（）]{1,50}?)\s*[)）]/g,(_,x)=>hold(sqrt(x))).replace(/√\s*([0-9٠-٩۰-۹]+(?:[.,٫][0-9٠-٩۰-۹]+)?)/g,(_,x)=>hold(sqrt(x))).replace(/√\s*([A-Za-z\u0600-\u06FF](?:\s*[+\-−×÷]\s*[A-Za-z0-9٠-٩۰-۹\u0600-\u06FF]+)?)/g,(_,x)=>hold(sqrt(x)));
  ...
  s=s.replace(/([0-9٠-٩۰-۹]+)\s*\^\s*([0-9٠-٩۰-۹]+)\s*([+\-−×÷])\s*([0-9٠-٩۰-۹]+)\s*\^\s*([0-9٠-٩۰-۹]+)/g,(_,a,b,op,d,e)=>hold(expr([pow(a,b),`<span class="qop">${esc(op)}</span>`,pow(d,e)]))).replace(/([0-9٠-٩۰-۹]+)([⁰¹²³⁴⁵⁶⁷⁸⁹]+)\s*([+\-−×÷])\s*([0-9٠-٩۰-۹]+)([⁰¹²³⁴⁵⁶⁷⁸⁹]+)/g,(_,a,b,op,d,e)=>{const m={'⁰':'٠','¹':'١','²':'٢','³':'٣','⁴':'٤','⁵':'٥','⁶':'٦','⁷':'٧','⁸':'٨','⁹':'٩'};const cv=x=>[...x].map(z=>m[z]||z).join('');return hold(expr([pow(a,cv(b)),`<span class="qop">${esc(op)}</span>`,pow(d,cv(e))]))});
  s=s.replace(/([A-Za-z\u0600-\u06FF0-9٠-٩۰-۹]+)\s*\^\s*[（(]\s*([^()（）]{1,40})\s*[)）]/g,(_,a,b)=>hold(pow(a,b)));
  s=s.replace(/([0-9٠-٩۰-۹]+)\s+([٢٣٤٥٦٧٨٩2-9])(?=\s*(?:[×*÷+=]))/g,(_,a,b)=>hold(pow(a,b))).replace(/([（(][^()（）]{1,40}[)）])\s*([٢٣23])(?=\s*(?:[=،,.؟?]|$))/g,(_,a,b)=>hold(pow(a,b))).replace(/(سم|كم|مم|م)\s*[\^]\s*([0-9٠-٩۰-۹]+)/g,(_,a,b)=>hold(pow(a,b))).replace(/(سم|كم|مم|م)([²³])/g,(_,a,b)=>hold(pow(a,b==='²'?'٢':'٣'))).replace(/([A-Za-z\u0600-\u06FF0-9٠-٩۰-۹]+|[（(][^()（）]{1,30}[)）])\s*[\^]\s*[（(]?\s*([+\-−]?[0-9٠-٩۰-۹]+)\s*[)）]?/g,(_,a,b)=>hold(pow(a,b))).replace(/([A-Za-z\u0600-\u06FF0-9٠-٩۰-۹]+|[（(][^()（）]{1,30}[)）])([⁰¹²³⁴⁵⁶⁷⁸⁹]+)/g,(_,a,b)=>{const m={'⁰':'٠','¹':'١','²':'٢','³':'٣','⁴':'٤','⁵':'٥','⁶':'٦','⁷':'٧','⁸':'٨','⁹':'٩'};return hold(pow(a,[...b].map(x=>m[x]||x).join('')))});
