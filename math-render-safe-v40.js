// Qudrat unified math renderer v84 — fractions, roots and powers.
(function(){
 ...
 function sqrt(x){return `<bdi dir="ltr" class="qmath-isolate"><span class="qsqrt"><span>${esc(ar(x))}</span></span></bdi>`}
 ...
 function render(raw){
  ...
  s=s.replace(/([0-9٠-٩۰-۹]+)\s*\^\s*([0-9٠-٩۰-۹]+)\s*([+\-−×÷])\s*([0-9٠-٩۰-۹]+)\s*\^\s*([0-9٠-٩۰-۹]+)/g,(_,a,b,op,d,e)=>hold(expr([pow(a,b),`<span class="qop">${esc(op)}</span>`,pow(d,e)])))
   .replace(/([0-9٠-٩۰-۹]+)([⁰¹²³⁴⁵⁶⁷⁸⁹]+)\s*([+\-−×÷])\s*([0-9٠-٩۰-۹]+)([⁰¹²³⁴⁵⁶⁷⁸⁹]+)/g,(_,a,b,op,d,e)=>{const m={'⁰':'٠','¹':'١','²':'٢','³':'٣','⁴':'٤','⁵':'٥','⁶':'٦','⁷':'٧','⁸':'٨','⁹':'٩'};const cv=x=>[...x].map(z=>m[z]||z).join('');return hold(expr([pow(a,cv(b)),`<span class="qop">${esc(op)}</span>`,pow(d,cv(e))]))});
